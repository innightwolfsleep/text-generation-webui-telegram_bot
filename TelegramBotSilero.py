...
class Silero:
    punctuation = r'[\s,.?!/)\'\]>]'
    alphabet_map = {
        "A": " Ei ", "B": " Bee ", "C": " See ", "D": " Dee ", "E": " Eee ",
        "F": " Eff ", "G": " Jee ", "H": " Eich ", "I": " Eye ", "J": " Jay ",
    ...
    voices = {
        "en": {
            "model": "v3_en",
            "male": ['en_2', "en_7", "en_9", "en_13", "en_15", "en_17", "en_19", "en_20", ],
            "female": ['en_0', "en_10", "en_11", "en_12", "en_14", "en_16", "en_18", "en_24", "en_25", "en_82", "en_85"]
        },
        "ru": {
            "model": "v3_1_ru",
            "male": ["aidar", "eugene", ],
    ...
    def get_audio(self, text: str, user_id: int, user: User):
        ...
        try:
            model, _ = torch.hub.load(repo_or_dir=self.silero_repo, model=self.model,
                                      language=user.language, speaker=user.silero_model_id)
            wav_path = str(user_id) + ".wav"
            text = self.preprocess(text)
            if len(text.replace(" ", "")) == 0:
                return None
            model.save_wav(text=self.preprocess(text),
                           audio_path=wav_path,
    ...
    def replace_roman(self, string):
        ...
        while True:
            ...
            result = result[0:start + 1] + str(self.roman_to_int(result[start + 1:end - 1])) + result[
                                                                                               end - 1:len(result)]
        return result
